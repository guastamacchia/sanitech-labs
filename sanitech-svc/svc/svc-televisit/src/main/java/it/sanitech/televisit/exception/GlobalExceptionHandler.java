 ...
 *
 * <p>Converte le eccezioni applicative e tecniche in risposte RFC 7807 (Problem Details),
 * garantendo un formato uniforme e facilmente consumabile lato client.</p>
 */
...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ProblemDetails> forbidden(DepartmentAccessDeniedException ex, HttpServletRequest request) {
    ...
    public ResponseEntity<ProblemDetails> forbidden(ForbiddenException ex, HttpServletRequest request) {
        return ResponseEntity.status(HttpStatus.FORBIDDEN).body(ProblemDetails.builder()
                .type(AppConstants.Problem.TYPE_FORBIDDEN)
                .title(AppConstants.ErrorMessage.ERR_FORBIDDEN)
                .status(HttpStatus.FORBIDDEN.value())
...
@ExceptionHandler(MethodArgumentNotValidException.class)
    ...
    public ResponseEntity<ProblemDetails> generic(Exception ex, HttpServletRequest request) {
        return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(ProblemDetails.builder()
                ...
                .instance(request.getRequestURI())
                .extra(ex.getClass().getSimpleName())
                .build());
    }
}
