...
class OutboxKafkaPublisherTest {
    ...
    static final PostgreSQLContainer<?> postgres = new PostgreSQLContainer<>("postgres:16-alpine")
            .withDatabaseName("sanitech_prescribing_test")
            .withUsername("test")
            .withPassword("test");
    ...
    void publishBatch_shouldMarkEventAsPublishedOnSuccess() throws Exception {
        OutboxEvent e = OutboxEvent.builder()
                .aggregateType(AppConstants.Outbox.AGGREGATE_PRESCRIPTION)
                .aggregateId("1")
                .eventType(AppConstants.Outbox.EVT_PRESCRIPTION_CREATED)
        ...
        String topic = AppConstants.Outbox.TOPIC_PRESCRIBING_EVENTS;
        RecordMetadata meta = new RecordMetadata(new TopicPartition(topic, 0), 0, 0, System.currentTimeMillis(), 0L, 0, 0);
        ProducerRecord<String, String> pr = new ProducerRecord<>(topic, "PRESCRIPTION:1", "{\"k\":\"v\"}");
